...
context("With solver $(typeof(solver))") do
    ...
    @constraint(m, :SDP, X <= 1/2*eye(3,3))
    @variable(m, Y[1:5,1:5], Symmetric)
    @constraint(m, :SDP, -ones(5,5) <= Y)
    @constraint(m, :SDP, Y <= 2*ones(5,5))
    @variable(m, Z[1:4,1:4], Symmetric)
    @constraint(m, :SDP, ones(4,4) >= Z)
...
function nuclear_norm(model, A)
    ...
    @constraint(model, :SDP, 0 ⪯ [U A; A' V])
...
function operator_norm(model, A)
    m, n = size(A,1), size(A,2)
    @variable(model, t >= 0)
    @constraint(model, :SDP, [t*eye(n) A; A' eye(n)*t] >= 0)
    return t
end
...
function lambda_max(model, A)
    m, n = size(A,1), size(A,2)
    @variable(model, t)
    @constraint(model, :SDP, speye(n)*t - A ⪰ 0)
    @constraint(model, :SDP, A >= 0)
    return t
end
...
function lambda_min(model, A)
    m, n = size(A,1), size(A,2)
    @variable(model, t)
    @constraint(model, :SDP, A - eye(n)*t >= 0)
    @constraint(model, :SDP, A >= 0)
    return t
end
