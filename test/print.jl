...
facts("[print] JuMPContainer{Variable}") do
    ...
    @variables(m, begin
        rng_unit1[1:10]     # Array{Variable}
        rng_unit2[-2:3]     # JuMPArray
        rng_unit3[[1:10;]]  # JuMPDict
        rng_step1[1:2:10]
        rng_step2[-2:5:10]
        rng_step3[1:5:3]
        rng_step4[0:2:2]
        arr_1[[:a,:b,:c]]
        arr_2[[:a,1,"test"]]
        arr_3[[:apple,:banana,:carrot,:diamonds]]
        rng2_1[1:10,[:a,:b,:c]]
        tri_1[i=1:3,j=i:3]
        tri_2[i=1:3,j=-i]
        tri_3[(i,j)=[(i,i+2) for i in 1:5],k=i:j]
    end)
...
Solver is default solver""", repl=:show)
    ...
    @constraints(mod_3, :nonlinear, begin
        x[1]*x[2] == 1
        x[3]*x[4] == 1
        x[5]*x[1] == 1
    end)
    @objective(mod_3, :nonlinear, Min, x[1]*x[3])
