...
facts("[macros] Check @addConstraint basics") do
    m = Model()
    @defVar(m, w) # test deprecated syntax
    @variable(m, x)
    @variable(m, y)
    @variable(m, z)
    t = 10

    @addConstraint(m, 3x - y == 3.3(w + 2z) + 5) # test deprecated syntax
...
facts("[macros] nonlinear @constraints") do
    m = Model()
    @variable(m, 0 <= x <= 1)
    @variable(m, y[1:3])
    @objective(m, Max, x)
...
facts("[macros] Vectors in nonlinear expressions") do
    m = Model()
    @variable(m, x[1:3])
    @fact_throws ErrorException @objective(m, :nonlinear, Min, x)
    @fact_throws ErrorException @objective(m, :nonlinear, Min, [1,2,3])
end
...
facts("[macros] Conditions in constraint indexing") do
    ...
    @constraint(model, :nonlinear, c2[i=["red","blue","green"], k=9:-2:2; (i == "red" && isodd(k)) || (k >=4 && (i == "blue" || i == "green"))], x[k]^3 <= 1)
